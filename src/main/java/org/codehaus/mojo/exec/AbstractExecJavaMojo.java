 ...
 * @since 1.0
 */
public abstract class AbstractExecJavaMojo
    extends AbstractExecMojo
{
    ...
    /**
     * Check if the execution should be forked
     *
     * @return true to fork
     */
        ...
        if ( isFork() ) {
            ...
            Thread bootstrapThread = new Thread( threadGroup, new Runnable()
            {
                public void run()
                {
                    ...
                    {
                        Method main =
                            Thread.currentThread().getContextClassLoader().loadClass( getMainClass() ).getMethod( "main",
                    ...
                    { // just pass it on
                        Thread.currentThread().getThreadGroup().uncaughtException( Thread.currentThread(),
                                                                                   new Exception(
                                                                                                  "The specified _mainClass doesn't contain a main method with appropriate signature.",
                                                                                                  e ) );
                    }
                    catch ( Exception e )
                    { // just pass it on
        ...
        throws MojoExecutionException
    {
        ...
        {
            setArguments(parseCommandlineArgs());
        }
    ...
    /**
     * Check if the execution should include the plugin dependencies
     */
    protected boolean isIncludePluginDependencies() {
        return this.includePluginDependencies;
    }

    /**
     * Check if the execution should include the project dependencies
     */
    protected boolean isIncludeProjectDependencies() {
